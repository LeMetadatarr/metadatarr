...
def test_pyfanedit_lookup_returns_none():
    ...
    result = provider.lookup(Signals(title="Alien", medium=Medium.MOVIE))
    assert result is None


def test_pyfanedit_media_set():
    provider = PyfaneditProvider()
    assert Medium.MOVIE in provider.media
...
def test_list_variants_returns_provider_entities():
    ...
    signals = Signals(title="Alien", medium=Medium.MOVIE)
...
def test_list_variants_emits_correct_variant_kinds():
    ...
    assert kinds[1001] == VariantKind.FANFIX.value
    assert kinds[1002] == VariantKind.FANMIX.value
    ...
    assert kinds[1006] == VariantKind.FANEDIT_SHORT.value
...
def test_resolve_include_variants_populates_relations():
    ...
    signals = Signals(title="Alien", medium=Medium.MOVIE, include_variants=True)
