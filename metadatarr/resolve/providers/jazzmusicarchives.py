"""Jazz Music Archives provider via :mod:`pyjazzmusicarchives` (optional dep).

Resolves a jazz artist to its stable jazzmusicarchives slug and emits an artist
(group) entity. Activates for ``PlaybackType.AUDIO`` signals tagged ``"jazz"``.

Keys written to :attr:`ExternalIds.extra`:

- ``jazzmusicarchives_artist`` — artist slug (canonical)
- ``jazzmusicarchives_url``    — canonical artist page
"""
...
LOG = logging.getLogger("metadatarr.resolve.providers.jazzmusicarchives")
...
class JazzMusicArchivesProvider(MetadataProvider):
    ...
    name = "jazzmusicarchives"
    playback_type = {PlaybackType.AUDIO}
    genre_filter = {"jazz"}

    def __init__(self) -> None:
        try:
            import pyjazzmusicarchives as _lib  # noqa: WPS433
            self._lib = _lib
    ...
    def lookup(self, signals: Signals) -> Optional[ProviderMatch]:
        ...
        try:
            ...
            LOG.warning("jazzmusicarchives search failed: %s", exc)
            return None
        if not hits:
            return None
        best = hits[0]
