"""Tidal track metadata scraper — migrated onto the engine.

Scans tidal.com/track/{id} pages sequentially and extracts metadata from
Open Graph tags. No API key or account required.

The Tidal track ID space is dense in the low tens of millions.
...
Run it::
    ...
    python -m metadatarr.scrapers tidal_tracks [--output DIR] [--delay SECS] [--limit N]
"""
...
@register
class TidalTracksSource(Source):
    name = "tidal_tracks"
    id_field = ""
    default_delay = 1.0
    ...
    def _fetch_track(self, tidal_id: int) -> Optional[Dict[str, Any]]:
        self.throttle.wait()
        try:
        ...
        return {
            ...
            "url": f"https://tidal.com/track/{tidal_id}",
        }
    ...
    def fetch(self, cursor: int) -> Page:
        ...
        for tidal_id in range(start, chunk_end):
            row = self._fetch_track(tidal_id)
            if row:
                rows.append(row)
...
if __name__ == "__main__":
    raise SystemExit(run_cli(TidalTracksSource))
